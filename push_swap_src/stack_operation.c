...
/*   Updated: 2024/08/11 02:07:56 by vodebunm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void stack_a_init(t_push_swap_stack **a, char **argv)
{
    ...
    while (argv[i])
    {
        ...
        if (format_check(argv[i]) ||
            ...
            rm_copy(*a, (int)value))//check
        {
            free_memory(a);  // Free the stack if any check fails //check
...
void turks_sorter(t_push_swap_stack **a, t_push_swap_stack **b)
{
    ...
    a_length = get_stack_len(*a); // Get the length of stack `a`
    
    if (a_length-- > 3 && !sorted_stack(*a))// If the length of `a` is greater than 3 and the stack `a` is not sorted, push the top element to stack `b`
        pb(b, a, false);
    ...
    while (a_length-- > 3 && !sorted_stack(*a))// While the length of `a` is greater than 3 and the stack `a` is not sorted, perform the following operations
    {
        turk_implement(*a, *b);
        a_b(a,b);
    }
    ...
    alt_sorter(a);
    while (*b) //Push elements back from `b` to `a` while `b` is not empty
    {
        turk_implement_b(*a, *b);
        b_a(a, b);
    }
    index_position(*a);
    data_value(a);//check smallest value athe top
}
